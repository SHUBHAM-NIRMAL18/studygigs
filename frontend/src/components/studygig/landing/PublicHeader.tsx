    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 items-center justify-between px-6 md:px-8">
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated ? (
            <Button size="sm" className="rounded-md px-5 text-sm shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-all" onClick={() => window.location.href = '/dashboard'}>
              Dashboard
            </Button>
          ) : (
            <>
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px]">
              ...
              <div className="flex flex-col gap-6 mt-8">
                ...
                <hr className="my-4" />
                {isAuthenticated ? (
                  <Button className="w-full" onClick={() => window.location.href = '/dashboard'}>Dashboard</Button>
                ) : (
                  <>
